...
def init_db():
    """ایجاد جدول‌ها در Supabase"""
    # بررسی وجود جدول expenses
    try:
        supabase.table("expenses").select("*").limit(1).execute()
    except:
        # جدول وجود ندارد - باید در Supabase Dashboard بسازید
    ...
    for category in DEFAULT_CATEGORIES:
        try:
            ...
            pass  # اگر وجود داشت نادیده بگیر
...
def get_range_summary(user_id, start_date, end_date):
    response = supabase.table("expenses").select("category").eq("user_id", user_id).gte("created_at", f"{start_date} 00:00:00").lte("created_at", f"{end_date} 23:59:59").execute()
    ...
    for row in response.data:
        ...
        # نیاز به کوئری جداگانه برای جمع
    return summary
